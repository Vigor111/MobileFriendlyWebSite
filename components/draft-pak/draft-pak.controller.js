    ...
    function DraftPakController($user, $q, $state, $players, $teams, $game, $dialog, PickWindowService) {
        ...
        function _initialize() {
            return $q.all([
                ...
                .then(function(values) {
                    ...
                    if (players.length ==0) {
                        vm.playerMsg = 'No players are available for selection. Check back next NFL week!';
                    }
        ...
        function playerAction(player) {
            var playerId = player.id,
                addingPick = !_isPlayerInPicks(playerId),
            ...
            return userMethod(playerId, vm.pakId, gameType)
                .then(function(res) {
        ...
        function removePick(pick) {
            ...
            return userPickRemove(playerId, vm.pakId, pick.api_type)
                .then(function(res) {
        ...
        function _isPlayerInPicks(playerId) {
        ...
        function search() {
            var query = vm.query.toLowerCase();
                vm.players = _players
                    .filter(function(p) {
                        return p.player.Name.toLowerCase().indexOf(query) > 0 || p.displayName.toLowerCase().indexOf(query) > 0;
                    });
        ...
        function _applyFilterToPlayer(p) {
            p.show = vm.filter === 'all' || p.player.position === vm.filter;
        }
